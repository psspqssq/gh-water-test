...
const { Item } = Form;
const FeedWaterInput = () => {
  const { hardness, conductivity, ph, temperature } =
  ...
  const handleSubmit = () => {
    if (hardness.value && conductivity.value && ph.value && temperature.value) {
  ...
  return (
    <Form labelCol={{ span: 4 }} wrapperCol={{ span: 14 }}>
      ...
      <Item label="Temperature">
        <Input {...temperature} style={formatting}></Input>
      </Item>
      <br />
